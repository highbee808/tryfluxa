/**
 * Admin Ingestion Resume API Endpoint
 * 
 * POST /api/admin-ingestion-resume - Resume content ingestion
 * 
 * Sets ingestion.enabled = true in content_config table.
  ...
  try {
    ...
    if (existing) {
      ...
      if (error) {
        ...
        sendError(res, 500, 'Database error', 'Failed to resume ingestion');
        return;
      }
    ...
    sendSuccess(res, {
      ...
      message: wasEnabled 
        ? 'Ingestion was already running' 
        : 'Ingestion has been resumed',
      config: result,
    });
  } catch (error: any) {
