...
export const ChatBox = ({
  ...
  return <>
      {/* Floating Button */}
      {!isOpen && <button onClick={() => setOpen(true)} className="fixed bottom-6 right-6 w-14 h-14 rounded-full bg-primary text-primary-foreground shadow-lg hover:scale-110 transition-transform flex items-center justify-center z-50" aria-label="Open chat">
          
        </button>}

      {/* Chat Panel */}
      {isOpen && <div className="fixed bottom-0 left-0 right-0 md:bottom-6 md:right-6 md:left-auto w-full md:w-96 h-[85vh] md:h-[32rem] bg-card border-t md:border border-border md:rounded-2xl shadow-2xl flex flex-col z-50 animate-fade-in-up">
          {/* Header */}
          <div className="flex items-center justify-between p-4 border-b border-border bg-primary/5 md:rounded-t-2xl">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                {isSpeaking && <p className="text-xs text-muted-foreground animate-pulse">Speaking...</p>}
              </div>
            </div>
          ...
          <ScrollArea className="flex-1 p-4" ref={scrollRef}>
            {messages.length === 0 ? <div className="text-center text-muted-foreground py-8 space-y-3 animate-fade-in">
                ...
                {messages.map((msg, idx) => <div key={idx} className={cn("flex animate-fade-in-up", msg.role === "user" ? "justify-end" : "justify-start")}>
                    <div className={cn("max-w-[80%] rounded-2xl px-4 py-2 text-sm", msg.role === "user" ? "bg-primary text-primary-foreground" : "bg-card text-foreground shadow-soft border border-border/20")}>
                      {msg.role === "fluxa" && <div className="flex items-center gap-2 mb-1">
                ...
                {isLoading && <div className="flex justify-start animate-fade-in-up">
          ...
          <div className="p-4 border-t border-border">
            <div className="flex gap-2">
              <Input value={input} onChange={e => setInput(e.target.value)} onKeyPress={handleKeyPress} placeholder="Type a message..." disabled={isLoading} className="flex-1" />
