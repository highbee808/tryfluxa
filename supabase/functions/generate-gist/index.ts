...
const sanitizeArticleText = (text?: string) => {
  if (!text) return ''
  return text
    .replace(/\[[^\]]*\]/g, '') // strip [bracketed] refs
...
const stableArticleId = (article: Article) => {
  ...
  hash.update(
    `${article.source || 'fluxa'}-${article.title || 'untitled'}-${
...
async function fetchGuardianArticles(topic: string): Promise<Article[]> {
  ...
  try {
    const url = `https://content.guardianapis.com/search?q=${encodeURIComponent(
      topic,
    )}&order-by=newest&page-size=5&show-fields=trailText,bodyText,thumbnail&api-key=${apiKey}`
    const response = await fetch(url)
    if (!response.ok) {
...
async function fetchMediastackArticles(topic: string): Promise<Article[]> {
  ...
  try {
    const url = `http://api.mediastack.com/v1/news?access_key=${apiKey}&keywords=${encodeURIComponent(
      topic,
    )}&languages=en&limit=5&sort=published_desc`
    const response = await fetch(url)
    if (!response.ok) {
...
async function summarizeArticle(
  ...
  if (!articleText) {
    return {
      context: sanitizeArticleText(article.description || article.title || topic).slice(
        0,
        220,
      ),
      script: sanitizeArticleText(
        article.content || article.description || article.title || topic,
      ).slice(0, 600),
    }
  }
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are Fluxa, a concise entertainment and culture editor. Turn article facts into bright, conversational summaries without adding any links.',
        },
        {
          role: 'user',
          content: `Topic: ${topic}\nSource: ${
  ...
  try {
    ...
    return {
      context: sanitizeArticleText(
        parsed.context || article.description || article.title || topic,
      ).slice(0, 220),
      script: sanitizeArticleText(
        parsed.script || article.content || article.description || article.title || topic,
      ),
    }
  } catch (error) {
